        ...
        for alpha in args.alpha:
            ...
            for mass_mulL in args.mass_muls:
                ...
                _, running_state = pickle.load(
                    open(os.path.join(
                        assets_dir("env" + args.env_name + "noise_var0.0"),
                        'expert_traj/' + args.env_name + "_expert_traj.p"),
                         "rb"))
                for seed in args.seeds:
                    means = []
                    data_subfolder = "env" + args.env_name + "noise_varL" + str(
                            mass_mulL) + "noise_varE0.0"
                    if not alg == "expert":
                        if os.path.getsize(
                        ...
                        policy_net, _, _ = pickle.load(open(os.path.join(
                            assets_dir(
                                "env" + args.env_name + "noise_var0.0/learned_models"),
                            args.env_name + "_ppo.p"), "rb"))
                        expert_flag = True
        ...
        if not args.best:
            pickle.dump((to_plot, to_plot_std, names),
                        open("../plot/" + subfolder + "/DataCompareAlphas"
                             + args.env_name + str(
                            args.seeds) +
                             args.alg[0] + "MEeval.p", 'wb'))
        else:
            pickle.dump((to_plot, to_plot_std, names),
                        open("../plot/" + subfolder + "/DataCompareAlphas"
                             + args.env_name + str(
                            args.seeds) +
                             args.alg[0] + "bestMEeval.p",
                             'wb'))
else:
    subfolder = "env" + str(args.env_name)
    if not args.best:
        to_plot_load, to_plot_std_load, names_load = pickle.load(
            open("../plot/" + subfolder + "/DataCompareAlphas"
                 + args.env_name + str(args.seeds) + "MEeval.p",
                 'rb'))
    else:
        to_plot_load, to_plot_std_load, names_load = pickle.load(
            open("../plot/" + subfolder + "/DataCompareAlphas"
                 + args.env_name + str(args.seeds) + "bestMEeval.p",
                 'rb'))
...
plot.plot_lines_and_ranges(list_to_plot=to_plot,
                           ...
                           title=args.alg[
                               ...
                               args.best) + str(args.seeds) + "MEeval",
