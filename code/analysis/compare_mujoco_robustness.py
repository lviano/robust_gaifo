...
if not args.no_compute:
    ...
    to_plot_std = []
    print("START")
    subfolder = "TestRobustenv" + str(args.env_name) + "friction" + str(args.friction) + "mismatch" + str(args.mismatch)
    ...
    for alg in args.alg:
        for alpha in args.alpha:
            ...
            for mass_mulL in args.mass_muls:
                ...
                for seed in args.seeds:
                    means = []
                    if not args.friction:
                        data_subfolder = "env" + args.env_name + "massL" + str(args.mismatch) + "massE1.0"
                    else:
                        data_subfolder = "env" + args.env_name + "frictionL" + str(args.mismatch) + "frictionE1.0"
                    if not alg == "expert":
        ...
        if not args.best:
            pickle.dump((to_plot, to_plot_std, names), open("../plot/" + subfolder + "/DataCompareAlphas"
                                                            + args.env_name + str(args.seeds) + "friction" + str(
                args.friction) + args.alg[0] + ".p", 'wb'))
        else:
            pickle.dump((to_plot, to_plot_std, names), open("../plot/" + subfolder + "/DataCompareAlphas"
                                                            + args.env_name + str(args.seeds) + "friction" + str(
                args.friction) + args.alg[0] + "best.p", 'wb'))
else:
